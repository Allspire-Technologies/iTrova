-- Extend the bell's sync_notifications with Production alerts:
--   * production_request  — owners/managers: a requisition is waiting for a decision,
--   * production_decided  — the requester: their request was approved or rejected (last 14 days).
-- Gated by a new notification preference `production_alerts` (defaults on). Full CREATE OR
-- REPLACE of the function from 20260703240000 with the two new blocks; the prefs read moves
-- above the manager gate because production_decided must reach non-manager requesters too.
...
begin
  ...
  -- Window: decided in the last 14 days ('completed' implies approved, keep it visible).
  ...
  if not is_mgr then
    delete from public.notifications
      where recipient_id = uid and type in ('low_stock','out_of_stock','overdue','expiring','plan_expiring','plan_expired','store_low_stock','store_out_of_stock','store_overdue','production_request');
    return;
  end if;

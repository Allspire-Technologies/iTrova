-- Production shipping/transport: a run can carry a shipping/transport cost separate from labour/
-- overhead. Both are added to the run cost (and so to the produced products' cost) and capitalised
-- into Inventory in the ledger. Whole-function replace of record_production_run (last set
-- 20260720100000) + a new _shipping arg; drops the old 6-arg signature so the 7-arg fully replaces it.

drop function if exists public.record_production_run(uuid, uuid, jsonb, jsonb, text, numeric);
...
create or replace function public.record_production_run(
  ...
  _labour_overhead numeric default 0, _shipping numeric default 0
) returns jsonb
...
begin
  ...
  -- Run cost = consumed materials at cost + labour/overhead + shipping/transport.
  ...
  v_run_cost := v_raw_cost + coalesce(_labour_overhead, 0) + coalesce(_shipping, 0);
  ...
  -- Ledger: capitalise labour/overhead + shipping into Inventory (raw→finished reclass is net-zero).
  v_added := coalesce(_labour_overhead, 0) + coalesce(_shipping, 0);
  if v_added > 0 then
    begin
      if exists (select 1 from public.accounts where business_id = _business_id) then
        perform public._post_journal_impl(_business_id, current_date, 'Production costs', 'production', v_run_id, jsonb_build_array(
          jsonb_build_object('account_code', '1200', 'debit', v_added, 'credit', 0, 'description', 'Labour/overhead + shipping into stock'),
          jsonb_build_object('account_code', '1000', 'debit', 0, 'credit', v_added, 'description', 'Cash')));
      end if;
...
$$;
revoke all on function public.record_production_run(uuid, uuid, jsonb, jsonb, text, numeric, numeric) from public, anon;
grant execute on function public.record_production_run(uuid, uuid, jsonb, jsonb, text, numeric, numeric) to authenticated;

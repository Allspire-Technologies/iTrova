-- Repoint the approver-facing production_request notification to the Raw Materials page: the
-- material-request approval leg now lives there (Raw Materials → Requests), so a custodian who
-- doesn't have the Production module can still reach it. Full CREATE OR REPLACE of the function
-- from 20260707130000 with only that one link changed (production_decided still points to
-- /production?tab=requests for the requester).
...
begin
  ...
  if want_production then
    ...
    select biz, uid, 'production_request',
           ...
           'production_requisition', r.id, '/raw-materials?tab=requests'
    from public.production_requisitions r

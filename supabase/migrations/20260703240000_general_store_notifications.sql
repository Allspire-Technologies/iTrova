-- Extend the bell's sync_notifications with General Store alerts (owners/managers):
--   * store items at/under their reorder level (low / out of stock),
--   * borrowed items past their due date (overdue).
-- Gated by a new notification preference `general_store_alerts` (defaults on). This is a full
-- CREATE OR REPLACE of the function from 20260630140000 with the two new blocks + pref added.
...
begin
  ...
  if not is_mgr then
    delete from public.notifications
      where recipient_id = uid and type in ('low_stock','out_of_stock','overdue','expiring','plan_expiring','plan_expired','store_low_stock','store_out_of_stock','store_overdue');
    return;
  end if;

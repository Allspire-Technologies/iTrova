-- Production run output cost price: a run may set the produced product's per-unit cost price
-- (production cost drifts batch to batch). When a cost is supplied for an output line it becomes the
-- product's new cost_price; when omitted the existing cost_price is retained.
--
-- Whole-function replace on top of 20260710100000 (waste) — the only change is the outputs loop:
-- it now reads an optional cost_price and coalesces it into products.cost_price.
...
begin
  ...
  -- Outputs: increment product stock (+ optionally set a new cost price) + audit rows.
  for v_out in
    select (e->>'product_id')::uuid as product_id, (e->>'quantity')::numeric as qty,
           (e->>'cost_price')::numeric as cost
    from jsonb_array_elements(_outputs) as e
  loop
    if v_out.qty is null or v_out.qty <= 0 then
      raise exception 'BAD_QUANTITY' using errcode = 'check_violation';
    end if;
    if v_out.cost is not null and v_out.cost < 0 then
      raise exception 'BAD_QUANTITY' using errcode = 'check_violation';
    end if;
    -- New cost price overrides when supplied; otherwise the current cost_price is kept.
    update public.products
       set stock_quantity = stock_quantity + v_out.qty,
           cost_price = coalesce(v_out.cost, cost_price)

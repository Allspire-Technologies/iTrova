-- Persist VAT through the POS commit path. Full CREATE OR REPLACE of commit_offline_sale
-- (20260629110000) — the only changes are `tax_amount` on the sales insert and `tax` on the invoice
-- insert, both from the new `_sale->>'tax'` (defaults 0). commit_pos_sale (20260706100000) is an
-- unchanged thin wrapper that forwards `_sale`, so it carries `tax` too. Offline sales send `tax` in
